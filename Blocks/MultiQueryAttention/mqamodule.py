...
class MobileMQA(nn.Module):
    def __init__(self, num_heads, wq_dim, wkv_dim, wp_dim, channels_in):
        super().__init__()
        self.num_heads = num_heads
        self.depth_kernel = nn.Parameter(torch.randn(channels_in, 1, 3, 3) * 0.01)
        self.depth_bias = nn.Parameter(torch.randn(channels_in, 1, 3, 3) * 0.01)
    ...
    def forward(self, input):
        downsampled = DepthwiseConv.apply(input, self.depth_kernel, self.depth_bias, 2, 1)
